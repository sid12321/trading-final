...
from training_progress import TradingProgressCallback, DataPreprocessingProgress
from gpu_accelerated_ppo import GPUAcceleratedPPO, create_optimized_ppo
from parameters import (
    VALUE_LOSS_BOUND, USE_PARALLEL_OPTIMIZATION, SIGNAL_OPTIMIZATION_WORKERS, DEVICE, USE_GPU_ACCELERATION
)

def cleanup_memory(aggressive=False):
    """Clean up memory for CPU efficiency"""
    gc.collect()

def safe_model_save(model, filepath, max_retries=3):
    """
    Safely save model with GPU memory management and error recovery
    """
    import os
    import time
    ...
    for attempt in range(max_retries):
        try:
            print(f"Attempting to save model (attempt {attempt + 1}/{max_retries})...")
            ...
            if "memory" in str(e).lower():
                print(f"⚠ Memory error during save attempt {attempt + 1}: {e}")
                cleanup_memory(aggressive=True)
                time.sleep(1)
                
                if attempt < max_retries - 1:
                    print("Retrying with CPU-only save...")
                    continue
            else:
                raise
        except Exception as e:
            print(f"⚠ Unexpected error during save attempt {attempt + 1}: {e}")
            if attempt < max_retries - 1:
                time.sleep(1)
                continue
            else:
...
def check_system_health():
    """
    Check system health (GPU/CPU and memory status)
    """
    import psutil
    ...
    try:
        ...
        if DEVICE == "cuda" and torch.cuda.is_available():
            ...
            status.update({
                ...
                'gpu_memory_used': f"{gpu_mem_alloc:.2f}GB",
                'gpu_memory_total': f"{gpu_mem_total:.2f}GB",
                'gpu_memory_percent': f"{(gpu_mem_alloc/gpu_mem_total)*100:.1f}%"
            })
        
        status['healthy'] = memory.percent < 90
        return True, status
...
def optimize_system_performance():
    """
    Optimize system performance (GPU/CPU and memory management)
    """
    print(f"🔧 Optimizing {DEVICE.upper()} performance...")
    ...
    try:
        ...
        if healthy and isinstance(status, dict):
            print(f"✓ {DEVICE.upper()} optimization successful")
            if DEVICE == "cuda":
                print(f"  GPU: {status.get('gpu_name', 'N/A')}")
                print(f"  GPU memory: {status.get('gpu_memory_used', 'N/A')} / {status.get('gpu_memory_total', 'N/A')}")
            print(f"  CPU cores: {status['cpu_cores']}")
            print(f"  System memory available: {status['memory_available']}")
            return True
        else:
            print(f"⚠ System unhealthy: {status}")
            return False
...
def optimize_device_settings():
    """Optimize device settings for maximum performance"""
    if DEVICE == "cuda":
        # GPU optimizations
        ...
        print(f"CPU optimization enabled for {N_CORES} cores")
  ...
  for SYM in SYMLISTGIVEN:
    for prefix in ["final"]:
      if(prefix=="final"):
        ...
        print(modelfilename)
        try:
          model = PPO.load(basepath + "/models/" + modelfilename)
...
def preprocess(kite): 
  ...
  for SYM in TESTSYMBOLS:
      ...
      # Generate optimized signals using MCMC/Simulated Annealing with CPU parallelism
      if GENOPTSIG:
        print(f"Generating optimized signals for {SYM}...")
        try:
            optimized_signals, enhanced_pnlframe = generate_optimized_signals_cpu(
                mldf, signalcolumns, 
      ...
      # CORRELATION FILTERING WITH UPDATED THRESHOLDS
      numeric_df = mldf[goodsignals['var'].tolist()]
      ts = time.time()
      cormat = numeric_df.corr()
      te = time.time()
      print(f"Correlation matrix computation time: {te-ts:.2f}s")
      
      #SID: Implement collider filtering here 
      
      # Select final signals based on correlation - UPDATED THRESHOLDS
      CORMATTHRESHOLD1 = 0.95  # Updated from np.nanquantile(cormat,0.8) - less restrictive
      CORMATTHRESHOLD2 = 0.99  # Updated from np.nanquantile(cormat,0.95) - very permissive
      
      # Fill NAs with median correlation of non-NA values
      median_corr = np.nanmedian(cormat.values)
      cormat = cormat.fillna(median_corr)
      ...
      for ss in goodsignals['var'].tolist():
        ...
        max_corr = max([abs(cormat.loc[ss, fs]) if ss in cormat.index and fs in cormat.columns else median_corr
      ...
      print(f"Correlation filtering results:")
      print(f"  - finalsignals (threshold {CORMATTHRESHOLD1}): {len(finalsignals)} signals retained")
      print(f"  - finalsignals2 (threshold {CORMATTHRESHOLD2}): {len(finalsignals2)} signals retained")
      print(f"  - Original signal count: {len(goodsignals)} signals")
      
      print(f"\nSelected signals (threshold {CORMATTHRESHOLD1}):")
      print(finalsignals[:10], "..." if len(finalsignals) > 10 else "")
      print(f"\nSelected signals (threshold {CORMATTHRESHOLD2}):")
      print(finalsignals2[:10], "..." if len(finalsignals2) > 10 else "")
...
def modeltrain(rdflistp,NEWMODEL=NEWMODELFLAG,SYMLISTGIVEN=TESTSYMBOLS,DELETEMODELS=True,SAVE_BEST_MODEL=True,lol=None):
  ...
  for SYM in SYMLISTGIVEN: 
    for prefix in ["final"]: 
      ...
      callbacks = CallbackList([checkpoint_callback, custom_logger, tensorboard_callback])
      ...
      if(prefix=="final"):
        ...
        try:
          if NEWMODEL:
            ...
            ppo_params = {
                ...
                'device': DEVICE,  # Use the detected/configured device
            ...
            print(f"Creating PPO model with {DEVICE.upper()}, network size: {policy_kwargs['net_arch']}")
            
            # Use GPU-accelerated PPO if available
            if DEVICE == "cuda" and USE_GPU_ACCELERATION:
                try:
                    model = create_optimized_ppo(env, **ppo_params)
                    print("✓ GPU-accelerated PPO initialized")
                except Exception as e:
                    print(f"⚠ GPU PPO failed: {e}, falling back to CPU")
        ...
        print(f"{DEVICE.upper()} training: {adaptive_timesteps} total timesteps ({adaptive_timesteps//N_ENVS} per env)")
        print(f"Environments: {N_ENVS}, Device: {DEVICE}, CPU cores: {N_CORES}")
          
        # Check system health before training
        healthy, status = check_system_health()
        if not healthy:
            print(f"⚠ System health issue detected before training: {status}")
        
        # Add progress bar callback to existing callbacks
        progress_callback = TradingProgressCallback(
            total_timesteps=adaptive_timesteps,
            symbol=f"{SYM}_{prefix}",
            verbose=1
        )
        
        # Combine callbacks if they exist
        if callbacks is not None:
            if hasattr(callbacks, '__iter__'):
                all_callbacks = list(callbacks) + [progress_callback]
            else:
                all_callbacks = [callbacks, progress_callback]
        else:
            all_callbacks = [progress_callback]
        
        # Add learning rate scheduling
        try:
            print(f"🚀 Starting model training for {SYM}_{prefix}...")
            print(f"   Total timesteps: {adaptive_timesteps:,}")
        ...
        while ((MEANREWARD < MEANREWARDTHRESHOLD) & RETRAIN):
          ...
          # Add progress bar for continued training
          continue_progress_callback = TradingProgressCallback(
              total_timesteps=continue_timesteps,
              symbol=f"{SYM}_{prefix}_continued",
              verbose=1
          )
          
          # Combine callbacks for continued training
          if callbacks is not None:
              if hasattr(callbacks, '__iter__'):
                  continue_callbacks = list(callbacks) + [continue_progress_callback]
              else:
                  continue_callbacks = [callbacks, continue_progress_callback]
          else:
              continue_callbacks = [continue_progress_callback]
          
          try:
              print(f"🔄 Continuing training for {SYM}_{prefix}...")
              print(f"   Additional timesteps: {continue_timesteps:,}")
              model.learn(
                  total_timesteps=continue_timesteps, 
                  callback=continue_callbacks,
        ...
        if SAVE_BEST_MODEL:
          ...
          if best_model_path:
            ...
            try:
              ...
              safe_model_save(model, basepath + "/models/" + modelfilename)
          else:
            print(f"No best model found, saving current model for {SYM}{prefix}")
            safe_model_save(model, basepath + "/models/" + modelfilename)
        else:
          # Just save the final model
          print(f"Saving final model to {basepath}/models/{modelfilename}")
          safe_model_save(model, basepath + "/models/" + modelfilename)
